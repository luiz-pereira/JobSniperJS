...
function makeCard(request){
			...
			<div class="card4">
				<div class="card4-inner">
					<div class="card4-front">
						...
						<div class='row'>
							<ul class="col" id='includes'>
								${includes}
							</ul>
							<ul class="col" id='excludes'>
								${excludes}
							</ul>
						</div>
						<span class="row position-absolute">
							...
							<small class="text-center">Jobs found:${jobCount}</small>
						</span>
						<a href=''class='border border-warning float-right' id='update-status-${request.id}'><small>Not Updated</small><a>
					</div>
	...
	document.getElementById(`update-status-${request.id}`).addEventListener("click", function(e){
		e.preventDefault()
		requestUpdate(request).then(() =>$(`#update-status-${request.id}`).text("Updated!"))
	})
}
